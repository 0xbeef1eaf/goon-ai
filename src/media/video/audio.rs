//! Audio playback thread for video
//!
...
/// Audio playback thread that processes packets from the demuxer
...
impl AudioPlaybackThread {
            ...
            .spawn(move || {
                smol::block_on(async move {
                    let output_channel_layout = match config.channels() {
                        ...
                        channels => {
                            eprintln!(
                                "Unsupported channel count: {}, falling back to stereo",
                                channels
                            );
    ...
    /// Send a packet to the audio thread for decoding
    ...
    /// Send a control command (pause/play)
...
/// Trait for forwarding samples from FFmpeg to cpal
...
/// Bridge between FFmpeg audio decoding and cpal output
